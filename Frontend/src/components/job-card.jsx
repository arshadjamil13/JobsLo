...
function JobCard({job,isMyJob= false,savedInit = false,onJobSaved=()=>{}}) {
    ...
    const{fn:fnSavedJobs,data:Savedjobs,loading:loadingSavedJobs,error:errorSavedJob} =useFetch(saveJob,{alreadySaved : saved})
    ...
    const handleSaveJob=async()=>{
     ...
     const { loading: loadingDeleteJob, fn: fnDeleteJob } = useFetch(
       deleteJob,
       {
         job_id: job.id,
       },
  ...
  return (
    <Card className="flex flex-col">
        ...
        <CardFooter className="flex gap-2">
            ...
            {!isMyJob && (<Button variant='outline' className="w-15" onClick={handleSaveJob} disable={loadingSavedJobs}>
